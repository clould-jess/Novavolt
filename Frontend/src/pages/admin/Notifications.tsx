...
import { ArchiveIcon, BellIcon, Building2Icon, CarFrontIcon, ClipboardListIcon, MailIcon } from 'lucide-react';
...
const iconFor = { FLEET_REQUEST: Building2Icon, RESERVATION_REQUEST: CarFrontIcon, RENTAL_REQUEST: ClipboardListIcon, CONTACT_MESSAGE: MailIcon };
const labelFor = { FLEET_REQUEST: 'Fleet', RESERVATION_REQUEST: 'Réservation', RENTAL_REQUEST: 'Demande de location', CONTACT_MESSAGE: 'Contact' };
...
export function AdminNotifications() {
  ...
  const load = useCallback(async () => { setLoading(true); try { setItems((await listAdminNotifications()).items.filter((item) => item.type !== 'FLEET_REQUEST')); } catch { showToast({ tone: 'error', title: 'Impossible de charger les notifications.' }); } finally { setLoading(false); } }, [showToast]);
  ...
  return <div className="flex flex-col gap-6"><PageHeading title="Notifications" description={`${unread} non lue${unread === 1 ? '' : 's'}`} action={<Button variant="secondary" onClick={() => void load()}>Rafraîchir</Button>} />
    {items.length === 0 ? <EmptyState icon={<BellIcon className="h-6 w-6" />} title="Aucune notification" body="Les nouvelles demandes apparaîtront ici." /> : <div className="grid gap-3">{items.map((item) => { const Icon = iconFor[item.type] ?? BellIcon; return <Card key={item.id} padding="md" className={item.readAt ? 'opacity-70' : 'border-action/40 bg-action/[0.03]'}><CardHeader title={<div className="flex items-center gap-2"><Icon className="h-4 w-4 text-action" /><span>{item.title}</span>{!item.readAt && <Badge tone="info">Nouveau</Badge>}</div>} description={item.preview} action={<span className="text-2xs text-muted">{dateLong(item.createdAt)}</span>} /><div className="mt-4 flex flex-wrap gap-2"><Button size="sm" variant="secondary" loading={busy === item.id} onClick={() => void markRead(item)} disabled={Boolean(item.readAt)}>Marquer comme lu</Button><Button size="sm" variant="secondary" iconLeft={<ArchiveIcon className="h-4 w-4" />} loading={busy === item.id} onClick={() => void archive(item.id)}>Archiver</Button><Badge tone="neutral">{labelFor[item.type] ?? 'Notification'}</Badge></div></Card>; })}</div>}</div>;
}
