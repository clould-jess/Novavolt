...
export function AdminRentalRequests() {
  ...
  const load = useCallback(async () => { setLoading(true); setLoadError(null); try { setItems(await listRentalRequests()); } catch { setItems([]); setLoadError('Impossible de charger les demandes de location.'); showToast({ tone: 'error', title: 'Impossible de charger les demandes de location.' }); } finally { setLoading(false); } }, [showToast]);
